...
public class MovimientoEntityRepository implements IMovimientoRepository {
    ...
    @Override
    public MovimientoDto save(Integer IdUsuario, MovimientoInsertDto movimientoInsertDto) {
        MovimientoEntity movimientoEntity = this.movimientoMapper.toEntity(movimientoInsertDto);

        UsuarioEntity usuario = new UsuarioEntity();

        usuario.setIdUsuario(IdUsuario);

        movimientoEntity.setUsuario(usuario);

        return this.movimientoMapper.toDtoMovimiento(this.crudMovimientoEntity.save(movimientoEntity));
    }
